...
PRIVILEGED_DATA __attribute__ (( used )) TCB_t * volatile pxCurrentTCB = NULL;
...
void vTaskStartScheduler( void )
{
	 ...
	 * */
	{
		...
		xReturn = xTaskCreate(	prvIdleTask,
				...
				&xIdleTaskHandle );
		/*lint !e961 MISRA exception, justified as it is not a
...
static char *prvWriteNameToBuffer(char *pcBuffer, const char *pcTaskName)
{
	...
	/* Pad the end of the string with spaces to ensure columns line up when
	   printed out. */
	for (N = strlen(pcBuffer); N < (size_t) (configMAX_TASK_NAME_LEN - 1); ++N)
	{
		pcBuffer[N] = ' ';
	}
...
void vTaskList(void)
{
	...
	UBaseType_t	uxArraySize, N;
	char		cStatus, pcBuffer[configMAX_TASK_NAME_LEN];
	uint32_t	ulTotalTime, ulRunPercentage;
	...
	if (pxTaskStatusArray != NULL) {
		...
		printf("TCB \tName                    \tState \tPrio \tFree \tRun\r\n");

		for (N = 0; N < uxArraySize; ++N) {

			switch (pxTaskStatusArray[N].eCurrentState) {
			...
			prvWriteNameToBuffer(pcBuffer, pxTaskStatusArray[N].pcTaskName);

			if (ulTotalTime != 0UL) {

				ulRunPercentage = pxTaskStatusArray[N].ulRunTimeCounter / ulTotalTime;
			}
			else {
				ulRunPercentage = 0;
			}
			...
			printf("%i \t%s \t%c \t%i \t%i \t%i%%\r\n",
					(int) pxTaskStatusArray[N].xTaskNumber,
					pcBuffer, cStatus,
					(int) pxTaskStatusArray[N].uxCurrentPriority,
					(int) pxTaskStatusArray[N].usStackHighWaterMark,
					(int) ulRunPercentage);
		}
...
#endif /* ( ( configUSE_TRACE_FACILITY == 1 ) && ( configUSE_STATS_FORMATTING_FUNCTIONS ==1 0 ) ) */
/*----------------------------------------------------------*/
