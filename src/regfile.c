...
static void
reg_proc_task(const reg_t *reg, int *lval, const int *rval)
{
	int			reg_ID;
...
static void
reg_proc_DRV_status(const reg_t *reg, int *lval, const int *rval)
{
	if (lval != NULL) {
...
static void
reg_proc_DRV_configure(const reg_t *reg, int *lval, const int *rval)
{
	if (lval != NULL) {
...
{
	const float             Zp_2 = (float) (pm.const_Zp * pm.const_Zp);

	if (lval != NULL) {

		*lval = reg->link->f * Zp_2;
	}
	else if (rval != NULL) {

		if (pm.const_E > M_EPS_F) {

			reg->link->f = (*rval) / Zp_2;
		}
	}
}
...
reg_proc_kg(const reg_t *reg, float *lval, const float *rval)
{
	...
	const float		ld_Q = ld_R * ld_R;

	if (lval != NULL) {

		if (ld_Q > M_EPS_F) {

			*lval = reg->link->f * Zp_2 / ld_Q;
		}
		else {
			*lval = 0.f;
		}
...
static void
reg_proc_auto_forced_maximal(const reg_t *reg, float *lval, const float *rval)
{
	if (lval != NULL) {
...
reg_proc_wattage(const reg_t *reg, float *lval, const float *rval)
{
	if (lval != NULL) {
		...
		*lval = reg->link->f;
		...
		reg->link->f = m_fabsf(*rval);
	}
}

static void
reg_proc_amperage(const reg_t *reg, float *lval, const float *rval)
{
	if (lval != NULL) {

		*lval = reg->link->f / pm.const_fb_U;
	}
}

static void
reg_proc_USEABLE(const reg_t *reg, int *lval, const int *rval)
{
	if (lval != NULL) {
...
static void
reg_proc_dbg_flux_EFg(const reg_t *reg, float *lval, const float *rval)
{
	float		A, B;
	int		irq;
...
static void
reg_proc_dbg_hfi_EFg(const reg_t *reg, float *lval, const float *rval)
{
	float		A, B;
	int		irq;
...
#include "app/apdefs.h"
		...
		case ID_PM_CONFIG_HFI_IMPEDANCE:
		...
		case ID_PM_CONFIG_LU_ESTIMATE_HFI:
			...
			switch (val) {
				...
				PM_SFI_CASE(PM_HFI_EIGEN);
				PM_SFI_CASE(PM_HFI_ON_KALMAN);
		...
		case ID_PM_CONFIG_ABI_FRONTEND:

			switch (val) {

				PM_SFI_CASE(PM_ABI_INCREMENTAL);
				PM_SFI_CASE(PM_ABI_ABSOLUTE);
...
const reg_t		regfile[] = {
	...
	REG_DEF(ap.task_PUSHTWO,,,		"",	"%i",	REG_CONFIG, &reg_proc_task, &reg_format_enum),
	...
	REG_DEF(pm.config_HFI_IMPEDANCE,,,	"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	...
	REG_DEF(pm.config_ABI_FRONTEND,,,	"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	...
	REG_DEF(pm.lu_gain_QF,,,		"",	"%2e",	REG_CONFIG, NULL, NULL),
	...
	REG_DEF(pm.detach_threshold_BASE,,,	"V",	"%3f",	REG_CONFIG, NULL, NULL),
	...
	REG_DEF(pm.const_E, _nm,,		"Nm/A",	"%4g",	0, &reg_proc_E_nm, NULL),
	...
	REG_DEF(pm.watt_iDC_maximal,,,		"A",	"%3f",	REG_CONFIG, &reg_proc_wattage, NULL),
	REG_DEF(pm.watt_wP_reverse,,,		"W",	"%1f",	REG_CONFIG, &reg_proc_wattage, NULL),
	REG_DEF(pm.watt_iDC_reverse,,,		"A",	"%3f",	REG_CONFIG, &reg_proc_wattage, NULL),
	...
	REG_DEF(pm.watt_lpf_wP, _A,,		"A",	"%1f",	REG_READ_ONLY, &reg_proc_amperage, NULL),
	...
	REG_DEF(pm.s_tolerance,,,	"rad/s",	"%2f",	REG_CONFIG, NULL, NULL),
	...
	REG_DEF(pm.x_weak_zone,,,		"rad",	"%2e",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.x_weak_zone, _mm,,		"mm",	"%3f",	0, &reg_proc_mmps, NULL),
	REG_DEF(pm.x_tolerance,,,		"rad",	"%2e",	REG_CONFIG, NULL, NULL),
	...
	REG_DEF(tlm, .dbg_flux_EF_g,, "g", "%2f", REG_READ_ONLY, &reg_proc_dbg_flux_EFg, NULL),
	REG_DEF(tlm, .dbg_hfi_EF_g,,  "g", "%2f", REG_READ_ONLY, &reg_proc_dbg_hfi_EFg, NULL),
