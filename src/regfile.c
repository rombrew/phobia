...
static void
reg_proc_net_NART(const reg_t *reg, int *lval, const int *rval)
{
	if (lval != NULL) {
...
static void
reg_proc_torque_pc(const reg_t *reg, float *lval, const float *rval)
{
	if (lval != NULL) {
...
static void
reg_proc_tune_MPPE(const reg_t *reg, float *lval, const float *rval)
{
	if (lval != NULL) {
...
static void
reg_proc_tune_forced(const reg_t *reg, float *lval, const float *rval)
{
	if (lval != NULL) {
...
static void
reg_proc_tune_loop_current(const reg_t *reg, float *lval, const float *rval)
{
	if (lval != NULL) {
...
static void
reg_proc_tune_loop_speed(const reg_t *reg, float *lval, const float *rval)
{
	if (lval != NULL) {
...
static void
reg_proc_debug_flux_dF_g(const reg_t *reg, float *lval, const float *rval)
{
	float		A, B;
	int		irq;
...
static void
reg_proc_debug_hfi_dF_g(const reg_t *reg, float *lval, const float *rval)
{
	float		A, B;
	int		irq;
...
static void
reg_proc_fa_revol_F(const reg_t *reg, float *lval, const float *rval)
{
...
static void
reg_proc_fa_revol_F_g(const reg_t *reg, float *lval, const float *rval)
{
	float			angle;

	if (lval != NULL) {

		reg_proc_fa_revol_F(reg, &angle, rval);
...
static void
reg_proc_fa_revol_F_mm(const reg_t *reg, float *lval, const float *rval)
{
	float			angle;
...
static void
reg_format_referenced_analog(const reg_t *reg)
{
	reg_format_referenced(reg, ID_AP_ANALOG_REG_ID);
}

static void
reg_format_referenced_idle(const reg_t *reg)
{
	reg_format_referenced(reg, ID_AP_IDLE_REG_ID);
}

static void
reg_format_gain_U(const reg_t *reg)
{
	float		U;

	U = (pm.flux_URE + reg->link->f * pm.flux_MPPE) * pm.const_E;

	printf("%1f (%3f V)", &reg->link->f, &U);
}
...
reg_format_enum(const reg_t *reg)
{
	...
	switch (reg_ID) {
		...
		case ID_NET_PIPE_0_MODE:
		case ID_NET_PIPE_1_MODE:
		case ID_NET_PIPE_2_MODE:
		case ID_NET_PIPE_3_MODE:
		...
		case ID_NET_FLASH_MODE:
		case ID_NET_PIPE_0_STARTUP:
		case ID_NET_PIPE_1_STARTUP:
		case ID_NET_PIPE_2_STARTUP:
		case ID_NET_PIPE_3_STARTUP:
		...
		case ID_NET_PIPE_0_PAYLOAD:
		case ID_NET_PIPE_1_PAYLOAD:
		case ID_NET_PIPE_2_PAYLOAD:
		case ID_NET_PIPE_3_PAYLOAD:
		...
		case ID_PM_FSM_ERRNO:
			...
			printf("(%s)", pm_strerror(pm.fsm_errno));
			break;
		...
		case ID_PM_CONFIG_LU_SENSOR_HALL:
		case ID_PM_CONFIG_LU_SENSOR_ABI:
		case ID_PM_CONFIG_LU_SENSOR_SINCOS:
		case ID_PM_CONFIG_FLUX_IMBALANCE:
		case ID_PM_CONFIG_HFI_MAJOR:
		case ID_PM_CONFIG_HOLDING_BRAKE:
		case ID_PM_CONFIG_SPEED_LIMITED:
		case ID_PM_CONFIG_MTPA_RELUCTANCE:
		...
		case ID_PM_CONFIG_FA_PRECEDENCE:

			switch (val) {

				TEXT_ITEM(PM_FA_INHERITED);
				TEXT_ITEM(PM_FA_ABI);
				TEXT_ITEM(PM_FA_SINCOS);
		...
		case ID_PM_FSM_STATE:
			...
			switch (val) {
				...
				TEXT_ITEM(PM_STATE_ADJUST_SENSOR_HALL);
				TEXT_ITEM(PM_STATE_ADJUST_SENSOR_SINCOS);
		...
		case ID_PM_LU_MODE:
			...
			switch (val) {
				...
				TEXT_ITEM(PM_LU_SENSOR_SINCOS);

				default: break;
			}
			break;

		case ID_PM_FLUX_MODE:

			switch (val) {

				TEXT_ITEM(PM_FLUX_UNCERTAIN);
				TEXT_ITEM(PM_FLUX_HIGH);
				TEXT_ITEM(PM_FLUX_LOCKED_IN_DETACH);
...
const reg_t		regfile[] = {
	...
	REG_DEF(hal.CAN_mode_NART,,,	"",	"%i", REG_CONFIG, &reg_proc_net_NART, &reg_format_enum),
	...
	REG_DEF(net.flash_MODE,,,	"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	REG_DEF(net.startup_LOST, _ms,,	"",	"%i",	REG_CONFIG, NULL, &reg_format_can_TIM_ms),

	REG_DEF(net.pipe, _0_ID, [0].ID,"",		"%i",	REG_CONFIG, &reg_proc_net_IDs, NULL),
	REG_DEF(net.pipe, _0_reg_DATA, [0].reg_DATA,"",	"%4e",	0, NULL, NULL),
	REG_DEF(net.pipe, _0_reg_ID, [0].reg_ID,"",	"%i",	REG_CONFIG | REG_LINKED, NULL, NULL),
	REG_DEF(net.pipe, _0_MODE, [0].MODE,"", "%i",	REG_CONFIG, &reg_proc_net_IDs, &reg_format_enum),
	REG_DEF(net.pipe, _0_STARTUP, [0].STARTUP,"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	REG_DEF(net.pipe, _0_tim_hz, [0].tim,	"Hz",	"%1f",	REG_CONFIG, &reg_proc_net_TIM, NULL),
	REG_DEF(net.pipe, _0_trigger_ID, [0].trigger_ID,"","%i",REG_CONFIG, &reg_proc_net_IDs, NULL),
	REG_DEF(net.pipe, _0_PAYLOAD, [0].PAYLOAD,"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	REG_DEF(net.pipe, _0_range_0, [0].range[0],"",	"%4e",	REG_CONFIG, NULL, NULL),
	REG_DEF(net.pipe, _0_range_1, [0].range[1],"",	"%4e",	REG_CONFIG, NULL, NULL),

	REG_DEF(net.pipe, _1_ID, [1].ID,"",		"%i",	REG_CONFIG, &reg_proc_net_IDs, NULL),
	REG_DEF(net.pipe, _1_reg_DATA, [1].reg_DATA,"",	"%4e",	0, NULL, NULL),
	REG_DEF(net.pipe, _1_reg_ID, [1].reg_ID,"",	"%i",	REG_CONFIG | REG_LINKED, NULL, NULL),
	REG_DEF(net.pipe, _1_MODE, [1].MODE,"", "%i",	REG_CONFIG, &reg_proc_net_IDs, &reg_format_enum),
	REG_DEF(net.pipe, _1_STARTUP, [1].STARTUP,"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	REG_DEF(net.pipe, _1_tim_hz, [1].tim,	"Hz",	"%1f",	REG_CONFIG, &reg_proc_net_TIM, NULL),
	REG_DEF(net.pipe, _1_trigger_ID, [1].trigger_ID,"","%i",REG_CONFIG, &reg_proc_net_IDs, NULL),
	REG_DEF(net.pipe, _1_PAYLOAD, [1].PAYLOAD,"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	REG_DEF(net.pipe, _1_range_0, [1].range[0],"",	"%4e",	REG_CONFIG, NULL, NULL),
	REG_DEF(net.pipe, _1_range_1, [1].range[1],"",	"%4e",	REG_CONFIG, NULL, NULL),

	REG_DEF(net.pipe, _2_ID, [2].ID,"",		"%i",	REG_CONFIG, &reg_proc_net_IDs, NULL),
	REG_DEF(net.pipe, _2_reg_DATA, [2].reg_DATA,"",	"%4e",	0, NULL, NULL),
	REG_DEF(net.pipe, _2_reg_ID, [2].reg_ID,"",	"%i",	REG_CONFIG | REG_LINKED, NULL, NULL),
	REG_DEF(net.pipe, _2_MODE, [2].MODE,"", "%i",	REG_CONFIG, &reg_proc_net_IDs, &reg_format_enum),
	REG_DEF(net.pipe, _2_STARTUP, [2].STARTUP,"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	REG_DEF(net.pipe, _2_tim_hz, [2].tim,	"Hz",	"%1f",	REG_CONFIG, &reg_proc_net_TIM, NULL),
	REG_DEF(net.pipe, _2_trigger_ID, [2].trigger_ID,"","%i",REG_CONFIG, &reg_proc_net_IDs, NULL),
	REG_DEF(net.pipe, _2_PAYLOAD, [2].PAYLOAD,"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	REG_DEF(net.pipe, _2_range_0, [2].range[0],"",	"%4e",	REG_CONFIG, NULL, NULL),
	REG_DEF(net.pipe, _2_range_1, [2].range[1],"",	"%4e",	REG_CONFIG, NULL, NULL),

	REG_DEF(net.pipe, _3_ID, [3].ID,"",		"%i",	REG_CONFIG, &reg_proc_net_IDs, NULL),
	REG_DEF(net.pipe, _3_reg_DATA, [3].reg_DATA,"",	"%4e",	0, NULL, NULL),
	REG_DEF(net.pipe, _3_reg_ID, [3].reg_ID,"",	"%i",	REG_CONFIG | REG_LINKED, NULL, NULL),
	REG_DEF(net.pipe, _3_MODE, [3].MODE,"", "%i",	REG_CONFIG, &reg_proc_net_IDs, &reg_format_enum),
	REG_DEF(net.pipe, _3_STARTUP, [3].STARTUP,"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	REG_DEF(net.pipe, _3_tim_hz, [3].tim,	"Hz",	"%1f",	REG_CONFIG, &reg_proc_net_TIM, NULL),
	REG_DEF(net.pipe, _3_trigger_ID, [3].trigger_ID,"","%i",REG_CONFIG, &reg_proc_net_IDs, NULL),
	REG_DEF(net.pipe, _3_PAYLOAD, [3].PAYLOAD,"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	REG_DEF(net.pipe, _3_range_0, [3].range[0],"",	"%4e",	REG_CONFIG, NULL, NULL),
	REG_DEF(net.pipe, _3_range_1, [3].range[1],"",	"%4e",	REG_CONFIG, NULL, NULL),
	...
	REG_DEF(ap.ppm_control_range, _0, [0],	"",	"%2f",	REG_CONFIG, NULL, &reg_format_referenced_ppm),
	REG_DEF(ap.ppm_control_range, _1, [1],	"",	"%2f",	REG_CONFIG, NULL, &reg_format_referenced_ppm),
	REG_DEF(ap.ppm_control_range, _2, [2],	"",	"%2f",	REG_CONFIG, NULL, &reg_format_referenced_ppm),
	...
	REG_DEF(ap.analog_control_ANG, _0, [0],	"",	"%2f",	REG_CONFIG, NULL, &reg_format_referenced_analog),
	REG_DEF(ap.analog_control_ANG, _1, [1],	"",	"%2f",	REG_CONFIG, NULL, &reg_format_referenced_analog),
	REG_DEF(ap.analog_control_ANG, _2, [2],	"",	"%2f",	REG_CONFIG, NULL, &reg_format_referenced_analog),
	REG_DEF(ap.analog_control_BRK,,,	"",	"%2f",	REG_CONFIG, NULL, &reg_format_referenced_analog),

	REG_DEF(ap.idle_ENABLED,,,		"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	REG_DEF(ap.idle_reg_ID,,,		"",	"%i",	REG_CONFIG | REG_LINKED, NULL, NULL),
	REG_DEF(ap.idle_control_tol, _0, [0],	"",	"%2f",	REG_CONFIG, NULL, &reg_format_referenced_idle),
	REG_DEF(ap.idle_control_tol, _1, [1],	"",	"%2f",	REG_CONFIG, NULL, &reg_format_referenced_idle),
	...
	REG_DEF(pm.config_LU_SENSOR_HALL,,,	"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	REG_DEF(pm.config_LU_SENSOR_ABI,,,	"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	REG_DEF(pm.config_LU_SENSOR_SINCOS,,,	"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	REG_DEF(pm.config_FA_PRECEDENCE,,,	"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	REG_DEF(pm.config_DRIVE,,,		"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	REG_DEF(pm.config_FLUX_IMBALANCE,,,	"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	REG_DEF(pm.config_HFI_MAJOR,,,		"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	...
	REG_DEF(pm.config_MTPA_RELUCTANCE,,,	"",	"%i",	REG_CONFIG, NULL, &reg_format_enum),
	...
	REG_DEF(pm.tvm_ALLOWED,,,"",	"%i",	REG_CONFIG | REG_READ_ONLY, NULL, &reg_format_enum),
	...
	REG_DEF(pm.lu_transition,,,	"rad/s",	"%2f",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.lu_lpf_torque,,,		"A",	"%3f",	REG_READ_ONLY, NULL, NULL),
	REG_DEF(pm.lu_gain_TF,,,		"",	"%2e",	REG_CONFIG, NULL, NULL),

	REG_DEF(pm.fa_F, _g,,		"g",	"%2f",	REG_READ_ONLY, &reg_proc_F_g, NULL),
	REG_DEF(pm.fa_revol, _F,,	"rad",	"%3f",	REG_READ_ONLY, &reg_proc_fa_revol_F, NULL),
	REG_DEF(pm.fa_revol, _F_g,,	"g",	"%2f",	REG_READ_ONLY, &reg_proc_fa_revol_F_g, NULL),
	REG_DEF(pm.fa_revol, _F_mm,,	"mm",	"%3f",	REG_READ_ONLY, &reg_proc_fa_revol_F_mm, NULL),

	REG_DEF(pm.forced_hold_D,,,		"A",	"%3f",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.forced_maximal,,,	"rad/s",	"%2f",	REG_CONFIG, &reg_proc_tune_forced, NULL),
	...
	REG_DEF(pm.flux_mode,,,			"",	"%i",	REG_READ_ONLY, NULL, &reg_format_enum),
	REG_DEF(pm.flux_lpf_wS,,,	"rad/s",	"%2f",	REG_READ_ONLY, NULL, NULL),
	REG_DEF(pm.flux_lpf_wS, _rpm,,		"rpm",	"%2f",	REG_READ_ONLY, &reg_proc_rpm, NULL),
	REG_DEF(pm.flux_lpf_wS, _mmps,,		"mm/s",	"%2f",	REG_READ_ONLY, &reg_proc_mmps, NULL),
	REG_DEF(pm.flux_lpf_wS, _kmh,,		"km/h",	"%1f",	REG_READ_ONLY, &reg_proc_kmh, NULL),
	REG_DEF(pm.flux_MPPE,,,		"rad/s",	"%2f",	REG_CONFIG, &reg_proc_tune_MPPE, NULL),
	REG_DEF(pm.flux_URE,,,		"rad/s",	"%2f",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.flux_gain_TAKE,,,		"",	"%1f",	REG_CONFIG, NULL, &reg_format_gain_U),
	REG_DEF(pm.flux_gain_GIVE,,,		"",	"%1f",	REG_CONFIG, NULL, &reg_format_gain_U),
	REG_DEF(pm.flux_gain_DE,,,		"",	"%2e",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.flux_gain_LP_S,,,		"",	"%2e",	REG_CONFIG, NULL, NULL),

	REG_DEF(pm.flux_imb_ONFLAG,,,		"",	"%i",	0, NULL, NULL),
	REG_DEF(pm.flux_imb_ripple_STD,,, "rad/s",	"%4f",	REG_READ_ONLY, NULL, NULL),
	REG_DEF(pm.flux_imb_KF, _0, [0],	"",	"%4e",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.flux_imb_KF, _1, [1],	"",	"%4e",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.flux_imb_KF, _2, [2],	"",	"%4e",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.flux_imb_KF, _3, [3],	"",	"%4e",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.flux_imb_KF, _4, [4],	"",	"%4e",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.flux_imb_KF, _5, [5],	"",	"%4e",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.flux_imb_KF, _6, [6],	"",	"%4e",	REG_CONFIG, NULL, NULL),
	...
	REG_DEF(pm.hfi_im_FP,,,			"",	"%2e",	REG_READ_ONLY, NULL, NULL),
	...
	REG_DEF(pm.hfi_FLUX,,,			"",	"%i",	REG_CONFIG, NULL, NULL),
	...
	REG_DEF(pm.hall_ST, _1, [1],	"g",	"%2f",	REG_READ_ONLY, &reg_proc_F_nolock_g, NULL),
	...
	REG_DEF(pm.hall_ST, _2, [2],	"g",	"%2f",	REG_READ_ONLY, &reg_proc_F_nolock_g, NULL),
	...
	REG_DEF(pm.hall_ST, _3, [3],	"g",	"%2f",	REG_READ_ONLY, &reg_proc_F_nolock_g, NULL),
	...
	REG_DEF(pm.hall_ST, _4, [4],	"g",	"%2f",	REG_READ_ONLY, &reg_proc_F_nolock_g, NULL),
	...
	REG_DEF(pm.hall_ST, _5, [5],	"g",	"%2f",	REG_READ_ONLY, &reg_proc_F_nolock_g, NULL),
	...
	REG_DEF(pm.hall_ST, _6, [6],	"g",	"%2f",	REG_READ_ONLY, &reg_proc_F_nolock_g, NULL),

	REG_DEF(pm.hall_ALLOWED,,,"",	"%i",	REG_CONFIG | REG_READ_ONLY, NULL, &reg_format_enum),
	...
	REG_DEF(pm.hall_prol_ms,,, 		"ms",	"%1f",	REG_CONFIG, NULL, NULL),
	...
	REG_DEF(pm, .debug_flux_dF_g,, "g", "%2f", REG_READ_ONLY, &reg_proc_debug_flux_dF_g, NULL),
	REG_DEF(pm, .debug_hfi_dF_g,, "g", "%2f", REG_READ_ONLY, &reg_proc_debug_hfi_dF_g, NULL),
	...
	REG_DEF(pm.const_Ja,,,		"A/rad/s2",	"%4g",	REG_CONFIG, NULL, NULL),
	...
	REG_DEF(pm.i_gain_P,,,		"",	"%2e",	REG_CONFIG, &reg_proc_tune_loop_current, NULL),
	...
	REG_DEF(pm.s_gain_P,,,		"",	"%2e",	REG_CONFIG, &reg_proc_tune_loop_speed, NULL),
	REG_DEF(pm.s_gain_S,,,		"",	"%2e",	REG_CONFIG, NULL, NULL),
	REG_DEF(pm.s_iSP,,,			"A",	"%3f",	REG_READ_ONLY, NULL, NULL),

	REG_DEF(pm.x_setpoint_F,,,		"rad",	"%3f",	0, &reg_proc_setpoint_F, NULL),
	...
	REG_DEF(pm.im_total_revol,,,		"",	"%i",	REG_CONFIG, NULL, NULL),
